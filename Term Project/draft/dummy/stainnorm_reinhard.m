...
% for i=1:x
%     for j=1:y
        result(:,:,1)=((source1(:,:,1)-m1)*(std4/std1))+m4;
        result(:,:,2)=((source1(:,:,2)-m2)*(std5/std2))+m5;
        result(:,:,3)=((source1(:,:,3)-m3)*(std6/std3))+m6;
%     end
% end
% for i=1:x
%     for j=1:y
%         result2(i,j,1)=((source1(i,j,1)-m1)*(std4/std1))+m4;
%         result2(i,j,2)=((source1(i,j,2)-m2)*(std5/std2))+m5;
%         result2(i,j,3)=((source1(i,j,3)-m3)*(std6/std3))+m6;
%     end
% end
norm_img=lab2rgb(result);   %Conversion of result from LAB to RGB space
figure(6)
imshow(uint8(norm_img)); %Output;stain normalized image
